...
class LastFM_Wrapper:
    ...
    def fetchArtistTags(self, artist, maxTagsToFetch, minWeight, retries=3):
        ...
        try:
            ...
            return filter(lambda pair: pair[0].lower().replace('the', '').strip() != artist.replace('the','').strip(), tags)        
        except Exception, err:
            if ('no artist' in str(err).lower()): return []
            if (retries > 0):
                common.safeStderr('Problem retrieving artist tag information for ['+artist+'], '+str(retries)+' retries left: '+str(err))
                time.sleep(5)
                return self.fetchArtistTags(artist, maxTagsToFetch, minWeight, retries - 1)
            else:
                common.safeStderr('Problem retrieving artist tag information for ['+artist+'], skipping: '+str(err))
        return None

        
    def fetchTrackTags(self, artist, track, maxTagsToFetch, minWeight, retries=3):
        ''' 
        ...
        try:
            ...
            return filter(lambda pair: pair[0].lower().replace('the', '').strip() != artist.replace('the','').strip() and pair[0].lower() != track, tags)
        except Exception, err:
            if ('no track' in str(err).lower() or 'no artist' in str(err).lower()): return []
            if (retries > 0):
                common.safeStderr('Problem retrieving track tag information for ['+unicode(artist)+':'+unicode(track)+'], '+str(retries)+' retries left: '+str(err))
                time.sleep(5)
                return self.fetchTrackTags(artist, track, maxTagsToFetch, minWeight, retries - 1)
            else:
                common.safeStderr('Problem retrieving track tag information for ['+artist+':'+track+'], skipping: '+str(err))
        return None
        ...
        try:
            ...
            for lastTopTag in lastTopTags:
                if (self.useNamedTuple):
                    ...
                    count = int(lastTopTag['weight'])
                if (key in tags):
                    common.safeStderr('Duplicate tag retrieved from lastFM, merging counts: '+lastTopTag)
                    tags[key] += count
                else:
                    tags[key] = count
            return tags
            ...
            if (retries > 0):
                common.safeStderr('Problem retrieving top tag information, '+str(retries)+' retries left: '+str(err))
                time.sleep(5)
                return self.fetchTopTagStats(retries - 1)
            else:
                common.safeStderr('Problem retrieving top tag information, '+str(retries)+' skipping: '+str(err))
        return None
    ...
    def fetchTagCount(self, tag, retries=3):
        ...
        try:
            ...
            if (retries > 0):
                common.safeStderr('Problem retrieving tag information, '+str(retries)+' retries left: '+str(err))
                time.sleep(5)
                return self.fetchTopTagStats(retries - 1)
            else:
                common.safeStderr('Problem retrieving tag information, '+str(retries)+' skipping: '+str(err))            
        return None
