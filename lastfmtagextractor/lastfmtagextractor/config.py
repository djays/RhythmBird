    ...
    def parseargs(self, argv=None):
        ...
        try:    
            ...
            for option in ['artistTagFields', 'trackTagFields', 'overwriteFields', 'forceOverwriteFields']:
                fields = set(map(string.strip, config.get(self.config_section, option).lower().split(',')))
                self.validFieldSet(option, fields, self.allowedMediaWriteFields)
            
            for option in ['genreSort', 'groupingSort', 'commentSort']:
                sorts = set(map(string.strip, config.get(self.config_section, option).lower().split(',')))
            ...
            print >> sys.stderr, os.path.basename(sys.argv[0]) + ': ' + str(err)
            print >> sys.stderr, self.usage_message
            sys.exit(-1)
