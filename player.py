...
#        self.open(self.__lib.loc[songid])
    ...
    # Generate Random next song in playlist
    def genRandom(self):
        i = randint(0,len(self.PlayQue))
        try:
            self.currSong = self.PlayQue.pop(i)
        except:
            return
    ...
    def playfromList(self):        
        ...
        self.state = 1     
        self.__ui.playlistTree.selectRow(self.currSong)        
    ...
    def __init_playlist__(self):
        ...
        # Shuffle Event
        QtCore.QObject.connect(self.__ui.b_Shuffle,SIGNAL("clicked()"),self.shuffle)
        self.isRandom = False
        self.PlayQue= []
    ...
    def shuffle(self):
        self.isRandom = not self.isRandom
        if self.isRandom == True:
            self.PlayQue = range(self.currSong, self.__ui.playlistTree.rowCount()-1)
